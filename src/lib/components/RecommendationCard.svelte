...
<article class="border border-ink bg-paper p-5 sm:p-6" aria-label="BLEP recommendation">
	<header class="flex flex-wrap items-start justify-between gap-4 border-b border-ink pb-5">
		<div>
			...
			<h3 class="mt-1 text-3xl font-bold">Gaming laptop shortlist</h3>
		</div>
				...
				<div class="border border-ink/35 bg-white p-4">
					<div class="flex flex-wrap items-start justify-between gap-3">
						<div>
							...
							<h5 class="mt-1 text-xl font-bold">{pick.name}</h5>
						</div>
						<p class="font-mono text-xs font-bold text-ink/65 uppercase">
							{formatIdr(pick.expected_price_idr)}
						</p>
					</div>
					<p class="mt-3 text-sm leading-relaxed font-bold text-ink/80">{pick.why}</p>
		...
		<div>
			...
			<div class="mt-3 grid gap-3">
				{#each recommendation.avoid as trap (trap.pattern)}
					<div class="border border-ink/35 bg-white p-4">
						<p class="font-bold">{trap.pattern}</p>
		...
		<div>
			...
			<ol class="mt-3 grid gap-3">
				{#each recommendation.deal_rules as rule (rule)}
					<li class="border border-ink/35 bg-white p-4 text-sm leading-relaxed font-bold">
						{rule}
					</li>
				{/each}
			</ol>
	...
	<footer class="mt-5 border-t border-ink/20 pt-5">
		...
		<p class="mt-2 text-base leading-relaxed font-bold">{recommendation.next_action}</p>
	</footer>
</article>
